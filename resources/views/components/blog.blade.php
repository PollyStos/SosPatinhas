        ...
        @foreach ($blog->blogs as $article)
            <div class="col-md-3 pb-4 galery_rounded my-5 mx-md-2 bg_color_light d-flex flex-column justify-content-between" style="max-width: 14rem">
                <div>
                    <div class="flex items-center space-x-2 mb-4 rounded-3">
                        <div class="overflow-hidden d-flex justify-content-center align-items-center">
                            <img src="{{ asset('img/' . $article->img) }}" class=" object-cover" style="max-height: 12rem">
                        </div>
                    </div>
                    <h4>{{ $article->title }}</h4>
                </div>
                ...
                <div class="mt-auto text-center">
                    <a href="{{ route('blog.show', $article->id) }}" class="btn bg_color_primary color_white mt-3">Ler
                        artigo</a>
                </div>
            </div>
        @endforeach
